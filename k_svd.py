    ...
    for i in range(0,1000):
        ...
        if abs(norm_diff) < tol:
            ...
            if index is None:
                ...
                with print_lock:
                    print threading.current_thread().name, ":" , Phi_list, Phi
                Phi_list = Phi
                with print_lock:
                    print threading.current_thread().name, ":" , Phi_list
                ...
                with print_lock:
                    print threading.current_thread().name, ":" , Phi_list[index], Phi
                with print_lock:
                    print threading.current_thread().name, ":" , Phi_list[index]
...
y_file = tb.open_file("y_small.h5", 'r')
y = y_file.root.data[:]
y_file.close()
